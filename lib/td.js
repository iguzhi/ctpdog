const logger = require('./logger').ctptd;

function Td(ctp, { privateTopicMode, publicTopicModel }) {
	this.ctp = ctp;
...
(function() {
	...
	this.OnFrontConnected = function() {
		let { investor, td, nRequestID, setting } = this.ctp;
		...
		if (!investor.tryLoginTimes || investor.tryLoginTimes < setting.maxTryLoginTimes) {
			...
			logger.info('ReqUserLogin : %s', td.ReqUserLogin(investor, nRequestID()));
