const logger = require('./logger').ctpmd;
...
(function() {
	this._init = function() {
		let { createMdApi, registerMdFront } = this.ctp;
		this._regsiterHandlers();

		createMdApi(this.ctp.accountID);
		registerMdFront();
	};
	...
	this.OnFrontConnected = function() {
	  logger.info('ReqUserLogin : %s', this.ctp.md.ReqUserLogin(this.ctp.getAccountByUserID(this.ctp.accountID), this.ctp.nRequestID()));
